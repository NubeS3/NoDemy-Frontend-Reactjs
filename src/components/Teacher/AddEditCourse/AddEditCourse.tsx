...
import {
    ...
    updateTeacherCourse,
    addTeacherCourse
} from '../../../reducers/teacherCourse.reducer';
    ...
    const handleUploadImageToImgur = async (e: any) => {
        try {
            ...
            reader.onload = function (event) {
                setCoverImage(event.target.result.toString());
            };
                            ...
                            <Paper className="AddEditCourse__paper">
                                ...
                                >
                                    ...
                                    }
                                    {
                                        isUploadingImage &&
                                        <div>
                                            <img className="AddEditCourse__cover-wrapper__loading" src={loading}
                                                 height={100} width={100} alt="Uploading..."/>
                                        </div>
                                    }
                                </div>
                                    ...
                                    >
                                        <MenuItem value=""><em>None</em></MenuItem>
                                        {
                                            Array.isArray(categories) && categories.map((_category) => (
                                                <MenuItem key={_category._id}
                                                          value={_category._id}>{_category.name}</MenuItem>
                                            ))
                                        }
                                    </Select>
                                </FormControl>
                                ...
                                <Grid container spacing={2} style={{marginTop: '20px'}}>
                                    ...
                                    <Grid item xs={6}>
                                        ...
                                        >
                                            {isModifying && <><img src={loadingSmall} alt=""
                                                                   height={23}/> &nbsp; </>} {history.location.pathname.includes('add') ? 'Add course' : 'Update course'}
                                        </Button>
                                    </Grid>
                                </Grid>
                            </Paper>
