...
export const editCategory = (data = {
    ...
    const response = await updateCatAPI(adminToken, id, data);
    if (!isResponseError(response)) {
...
export const deleteCategory = (id: string): AppThunk => async (dispatch, getState) => {
    const state = getState();
    const {adminAuthorizationReducer, categoriesReducer} = state;
    const {adminToken} = adminAuthorizationReducer;

    const response = await deleteCatAPI(adminToken, id);
    if (!isResponseError(response)) {
